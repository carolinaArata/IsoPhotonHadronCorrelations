...
#IsoGammaHadron(float ptTrMin, float ptTrMax, TString sFileDirShSig = directory where the trains for the analysis are, bMirror = Turn Mirror ON/OFF, TString shshBkg = "0.40-1.00", TString dirFiles = directory to save output files, double systPur = for Systematic on Purity: 1, 0.9, 1.1, bool bZYAM = Turn ZYAM analysis ON/OFF , bool bPlot = true, double phiMin, double phiMax, bool systShSh = ON/OFF, bool systTrackIneff = ON/OFF, bool systNMix18 = ON/OFF, bool systNMix45 = ON/OFF)
...
root -b -l <<EOF
.L ~/work/histogram/IsoPhotonHadronCorrelations/IsoGammaHadron.C
IsoGammaHadron(18, 40, "~/work/histogram/DataSh100_AssocPt500", true, "0.40-1.00", "~/work/histogram/FromScratch/checkCodeZYAM", 1, false, true, TMath::Pi() * 3 / 5, TMath::Pi(), false, false, false, false)
.q
EOF
